...
REM Add this classpath so servers will pick up the summing function class
set CLASSPATH=%CLASSPATH%;..\..\target\classes

echo Starting Locator and Server with partitioned regions ...
call gfsh run --file=serverStartPartitioned.gf
...
echo Running loader...
rem Engages the pdx profile, which will use PDX serialization
java -Dspring.profiles.active=pdx com.gopivotal.training.LoadRegions 
